...
void MainWindow::updatePreview()
{
    ...
    if (headerRows)
    {
        ...
        {
            ui->filePreviewTable->setHorizontalHeaderItem(n, new QTableWidgetItem(QString(headers.at(n))));
        }
    }
    ...
    for (int column = 0; column < columnsCount; column++)
    {
        ...
        {
            ui->filePreviewTable->setCellWidget(row, column, new QLabel(rows.at(row + headerRows).at(column)));
        }
    }
}
...
void MainWindow::on_firstRowCheckBox_stateChanged(int arg1)
{
    updatePreview();
}
...
{
    ui->ImportButton->setEnabled(ui->dateColumnComboBox->currentIndex() > 0 && ui->conceptColumnComboBox->currentIndex() > 0 && ui->amountColumnComboBox->currentIndex() > 0);
}
...
{
    QMessageBox box = QMessageBox(QMessageBox::Icon::Information, QString("tachiiin"), QString("tachaaan"));
    box.exec();
}
